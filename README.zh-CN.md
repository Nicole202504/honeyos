...
安装完成后，HoneyOS 会启动本地服务并自动打开首次设置网页。整个过程不需要在终端输入 API Key。

```text
模型服务：OpenAI / OpenRouter / DeepSeek / 自定义兼容接口
API Key：只在本地网页中填写，保存后不会回显
模型：自动读取后直接选择，也可以手动填写 Model ID
```

HoneyOS 会先从服务商读取当前 Key 可用的模型。若自定义接口没有实现 `/models`，可以手动填写 Model ID；HoneyOS 仍会立即发送一条极小的工具调用测试。只有 API Key、模型和 OpenAI Chat Completions 工具调用都可用，才会保存设置并继续。

网页的“设置 → 模型”使用同一套逻辑：选择服务商、读取可用模型、搜索选择，然后验证并保存。API Key 不会回显到页面。

### 2. 选择聊天入口

电脑网页始终可用。首次设置网页里还可以直接扫码连接微信或飞书，也可以暂时跳过。连接渠道后 HoneyOS 会自动重启一次，不需要返回终端。
...
可以在首次设置或“设置”页面扫码自动创建机器人。HoneyOS 使用 WebSocket 连接，不要求公网回调地址。首次私聊会得到配对码，由主人在本机执行：
