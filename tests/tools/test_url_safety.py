...
class TestProxyFakeIpCompatibility:
    """A proxy Fake-IP may stand in for a public hostname, never a literal target."""

    @pytest.mark.parametrize("fake_ip", ["198.18.0.14", "::ffff:0:c612:e"])
    def test_public_hostname_resolving_to_benchmark_ip_allowed(self, fake_ip):
        cfg = {"security": {"allow_proxy_fake_ips": True}}
        with patch("hermes_cli.config.read_raw_config", return_value=cfg), _resolves_to(fake_ip):
            assert is_safe_url("https://github.com/Panniantong/Agent-Reach") is True

    def test_literal_benchmark_ip_remains_blocked(self):
        cfg = {"security": {"allow_proxy_fake_ips": True}}
        with patch("hermes_cli.config.read_raw_config", return_value=cfg):
            assert is_safe_url("https://198.18.0.14/") is False

    @pytest.mark.parametrize("ip", ["127.0.0.1", "10.0.0.8", "192.168.1.8"])
    def test_other_private_ranges_remain_blocked(self, ip):
        cfg = {"security": {"allow_proxy_fake_ips": True}}
        with patch("hermes_cli.config.read_raw_config", return_value=cfg), _resolves_to(ip):
            assert is_safe_url("https://public-looking.example/") is False

    def test_cloud_metadata_remains_blocked(self):
        cfg = {"security": {"allow_proxy_fake_ips": True}}
        with patch("hermes_cli.config.read_raw_config", return_value=cfg):
            assert is_safe_url("http://169.254.169.254/latest/meta-data/") is False

    def test_connect_time_guard_uses_same_narrow_exception(self):
        cfg = {"security": {"allow_proxy_fake_ips": True}}
        with patch("hermes_cli.config.read_raw_config", return_value=cfg), _resolves_to("198.18.0.14"):
            assert _resolved_http_connect_ips("github.com", 443, "https") == ["198.18.0.14"]
