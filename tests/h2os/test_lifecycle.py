...
from h2os_cli.runtime import gateway_argv, run_gateway_command
...
def test_gateway_command_has_explicit_home_and_python(monkeypatch, tmp_path):
    ...
    assert captured.argv[1:] == ["-m", "hermes_cli.main", "gateway", "status"]
    assert captured.env["HERMES_HOME"] == str(tmp_path.resolve())
    assert captured.env["H2OS_HOME"] == str(tmp_path.resolve())
